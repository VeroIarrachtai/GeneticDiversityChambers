...
kin_matrixGBS  <- kinshipGBS$kinship

# Asignar nombres a filas y columnas (importante para identificación)
rownames(kin_matrixGBS) <- colnames(kin_matrixGBS) <- kinshipGBS$sample.id
...
orden_columnas <- colnames(kin_matrixGBS)[p$tree_col$order]
...
info_GBSMon <- read_csv("../../SecuencingData/outputs/df_GBS_metadata.csv")  # Usa el número o nombre de la hoja

# Crear vectores con muestras de interes

# Filtra las filas donde 'treatment' contiene "Chambers" y extrae la columna 'Simple_Name_Plate'
...
```{r}
#Descartar muestras
samples_to_remove <- c(forestRef, Gametophyte, Transcriptomic)

# Filtrar la matriz eliminando las muestras en filas y columnas
kin_matrixRNat_Cham <- kin_matrixGBS[!rownames(kin_matrixGBS) %in% samples_to_remove, 
                                !colnames(kin_matrixGBS) %in% samples_to_remove]
```
...
#Descartar muestras
samples_to_remove <- c("AE22_N04TAF","D65_N7P18","H53_N6P12","A115_N12P40","A135_N08P46",
...
pheatmap(kin_matrixRNat_ChamWO, 
         display_numbers = FALSE, 
         color = colorRampPalette(c("blue", "white", "red"))(100),
         main = "Matriz de Parentesco (IBD) con RNatural y Chambers WO",
...
### Plot relatedness con RNatural y Chambers WO RNAT
...
pheatmap(kin_matrixRNat_ChamWORNat, 
         display_numbers = FALSE, 
         color = colorRampPalette(c("blue", "white", "red"))(100),
         main = "Matriz de Parentesco (IBD) WO RNAT",
...
pheatmap(kin_matrixRNat_ChamWOCham, 
         display_numbers = FALSE, 
         color = colorRampPalette(c("blue", "white", "red"))(100),
         main = "Matriz de Parentesco (IBD) WO CHAMBERS",
...
p <-pheatmap(kin_matrixRNat_ChamWOCham, 
         display_numbers = FALSE, 
         color = colorRampPalette(c("blue", "white", "red"))(100),
         main = "Matriz de Parentesco (IBD) WO Cham",
         legend = TRUE,          # Asegurarse de que la leyenda esté activa
         legend_height = 0.5)
